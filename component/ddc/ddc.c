...
void add_to_list(uint8_t *buf)
{
    ...
    ebox_printf("<<<<add frame to send lisk ok ;id = %d\tget free = %d\r\n",frame_id,ebox_get_free());
    ...
    if(ack == 1)
    {
        ...
        ebox_printf("<<<<add frame to WaitAck lisk ok ;id = %d\tget free = %d\r\n",frame_id,ebox_get_free());
...
{
    uint16_t frame_len = data_len + 6 + 4;
    uint8_t *buf = (uint8_t*)ebox_malloc(frame_len);
    DdcNode_t *node_frame;
//    unsigned char buf[DDC_MAX_LENGTH];
    uint16_t i = 0,k = 0; 
    uint8_t m;
...
void print_frame(uint8_t *frame)
{
    int i;
    uint8_t *p = frame;
    uint16_t len = get_len_frome_frame(p);
    for(i = 0; i < len;i++)
