...
#define __GPIOA_CRL               0x00009900
#define __GPIOA_CRH               0x00000990
#define __GPIOB_CRL               0x00000000
#define __GPIOB_CRH               0x00079900
...
__inline static void stm32_UsartSetup (void) {
  ...
  if (__USART_USED & 0x01) {                                // USART1 used
    ...
    if      ((__USART1_REMAP & 0x04) == 0x00) {             // USART1 no remap
      ...
      GPIOA->CRH   |=  (0x08UL  << 8);                      // USART1 Rx (PA10) input pull-up
      GPIOA->BSRR   =  (1 << 10);
    }
    else {                                                  // USART1    remap
      RCC->APB2ENR |= RCC_APB2ENR_AFIOEN;                   // enable clock for Alternate Function
      ...
      GPIOB->CRL   |=  (0x08UL  << 28);                     // USART1 Rx (PB7) input pull-up
      GPIOB->BSRR   =  (1 << 7);
    }
    ...
    //USART1->CR1 |= USART_CR1_UE;                            // USART enable  ---no for peter
  } // end USART1 used
  ...
  if (__USART_USED & 0x02) {                                // USART2 used
    ...
    if      ((__USART2_REMAP & 0x08) == 0x00) {             // USART2 no remap
      ...
      GPIOA->CRL   |=  (0x08UL  << 12);                     // USART2 Rx (PA3)  input pull-up
      GPIOA->BSRR   =  (1 << 3);
      ...
      GPIOD->CRL   |=  (0x08UL  << 24);                     // USART2 Rx (PD6)  input pull-up
      GPIOD->BSRR   =  (1 << 6);
      if (__USART2_FLOWCTRL & 0x0300) {                     // HW flow control enabled
    ...
    if (__USART_INTERRUPTS & 0x02) {                        // interrupts used
      ...
      NVIC->ISER[1] |= (1 << (0x26 & 0x1F));   // enable interrupt
    }

    //USART2->CR1 |= USART_CR1_UE;                            // USART enable	---no for peter
  } // end USART2 used
  ...
  if (__USART_USED & 0x04) {                                // USART3 used
    ...
    if      ((__USART3_REMAP & 0x30) == 0x00) {             // USART3 no remap
      ...
      GPIOB->CRH   |=  (0x08UL  << 12);                     // USART3 Rx (PB11) input pull-up
      GPIOB->BSRR   =  (1 << 11);
      ...
      GPIOC->CRH   |=  (0x08UL  << 12);                     // USART3 Rx (PC11) input pull-up
      GPIOC->BSRR   =  (1 << 11);
      if (__USART3_FLOWCTRL & 0x0300) {                     // HW flow control enabled
        RCC->APB2ENR |=  RCC_APB2ENR_IOPBEN;                // enable clock for GPIOB
        GPIOB->CRH   &= ~(0xFFUL  << 20);                   // Clear PB13, PB14
      ...
      GPIOD->CRH   |=  (0x08UL  <<  4);                     // USART3 Rx (PD9) input pull-up
      GPIOD->BSRR   =  (1 << 9);
      if (__USART3_FLOWCTRL & 0x0300) {                     // HW flow control enabled
    ...
    if (__USART_INTERRUPTS & 0x04) {                        // interrupts used
      ...
      NVIC->ISER[1] |= (1 << (0x27 & 0x1F));   // enable interrupt
    }

    //USART3->CR1 |= USART_CR1_UE;                            // USART enable	---no for peter
  } // end USART3 used
...
void stm32_Init () {
  ...
  stm32_TimerSetup ();
	...
	RCC->APB2ENR |= RCC_APB2ENR_AFIOEN;                     // enable clock for Alternate Function
	AFIO->MAPR = (AFIO->MAPR & (~(0x7 << 24))) | (0x2 << 24);		//add by gopeter,disable JTAG but SW-DP
